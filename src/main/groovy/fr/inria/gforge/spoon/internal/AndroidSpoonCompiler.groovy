 ...
 */
public class AndroidSpoonCompiler extends JDTBasedSpoonCompiler {

    public AndroidSpoonCompiler(Factory factory) {
        super(factory);
    }
    ...
    protected void generateProcessedSourceFilesUsingCUs() throws Exception {
        ...
        for (spoon.reflect.cu.CompilationUnit cu : getFactory().CompilationUnit()
            ...
            for (File gen : generatedDirectories) {
                if (cu.getFile().getAbsolutePath().startsWith(gen.getAbsolutePath()))
                    continue printing;
            }
